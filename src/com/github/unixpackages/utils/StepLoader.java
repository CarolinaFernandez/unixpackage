...
public class StepLoader {
	...
	public static final Collection<String> steps = Constants.STEPS_METHODS.values();
	 ...
	 * 
	 * @param currentClass class being displayed in the panel
	...
	public static Class<CommonStep> getStep(String className) {
		...
		String classPackage = StepLoader.class.getPackage().getName().replace(".utils", ".steps");
	...
	/**
	 * Performs operations bound to a specific step number.
	 * E.g. generates or deletes appropriate temporary files.
	 * 
	 * @param stepNumber number of active step
	 */
	public static void performOperationByStepNumber(int stepNumber) {
		...
		if (stepNumber == 1) {
			...
			if (!result) {
				try {
					System.out.println("Cleaning temporal folder because of some previous error...");
					Shell.postProcess();
				} catch (Exception e) {
					System.out.print("Program exiting due to failure. Exception: " + e);
					System.exit(0);
				}
			}
		}
