...
public class SetPackageSources extends CommonStep {
	...
	private TablePanel gregsPanel = new TablePanel(fillAndRetrieveFileList(), columnNames);
	...
	public SetPackageSources() {
		...
		// Populate the panel
		JLabel splashLabel = new JLabel("Choose the sources to be added to the package");
		...
		removePathTupleFromList.addActionListener (new ActionListener() {
			public void actionPerformed (ActionEvent e) {
				...
				if (chosenPaths.length > 0) {
					...
					while (i >= 0) {
						Variables.PACKAGE_SOURCE_INSTALL_PAIRS.remove(chosenPaths[i]);
						i--;
					}
					updateSourceInstallPairsList();
					// After removing an entry in the table, focus is lost; thus button shall be disabled
					removePathTupleFromList.setEnabled(false);
				}
			}
		});
		...
		// Focus listener on the JTable to ensure that the "Remove" button updates properly
		...
		layout.setHorizontalGroup(
				...
				.addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING)
					.addComponent(gregsPanel.getMainPanel())
					)
				)
				;
	...
	public static <T extends JComponent> T findParent(JComponent comp, Class<T> clazz)  {
		if (comp == null) {
			return null;
		}
		if (clazz.isInstance(comp)) {
			...
			return SetPackageSources.findParent((JComponent) comp.getParent(), clazz);
		}     
	}
	...
	private void setInstallPairsInFileList() {
		try {
			...
			if (Variables.PACKAGE_SOURCE_INSTALL_PAIRS == null) {
				// If PACKAGE_SOURCE_INSTALL_PAIRS not set, fill with empty array
			...
			System.out.println("Could not find source/install pairs...? More info: " + e);
		}
	}
	...
	private void updateSourceInstallPairsList() {	
		...
		if (gregsPanel != null) {
			gregsPanel.setTableModelDataVector(Variables.PACKAGE_SOURCE_INSTALL_PAIRS, columnNames);
