...
public class EditPackageFiles extends CommonStep {
	...
	public EditPackageFiles() {
		...
		JLabel splashLabel = new JLabel(
				"Edit any package file by double-clicking it");
		this.add(splashLabel, BorderLayout.CENTER);
		File dir;

		// Regenerate temporary files if needed (e.g. folder deleted
		// accidentally)
		...
		try {
			...
			System.out
					.println("E: Could not find UNIX package files. Exception: "
							+ e);
		}
		...
		fileChooser.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent evt) {
				...
				if (evt.getClickCount() >= 2) {
					...
					if (Variables.PACKAGE_TYPE.equals("RPM")) {
						commandList.add(Constants.TMP_PACKAGE_REDHAT_FILES_PATH
								+ "/" + list.getModel().getElementAt(index));
					} else {
						commandList.add(Constants.TMP_PACKAGE_DEBIAN_FILES_PATH
								+ "/" + list.getModel().getElementAt(index));
					}
