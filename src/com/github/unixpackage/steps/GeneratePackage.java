...
public class GeneratePackage extends CommonStep {
	...
	public GeneratePackage() {
		...
		splashLabel = new JLabel("Your " + Variables.PACKAGE_TYPE
				+ " package will be shown in a new browser.");
		this.add(splashLabel, BorderLayout.CENTER);
		...
		JLabel rerunLabel = new JLabel(
				"To generate a new package, rerun the application");
		...
		// Set black colour for background, green colour for text, and set
		// border
		...
		ActionListener actionListener = new GenerateSourcesListener(
				textareaScrollPane, textArea, rerunLabel);
	...
	public static StringBuilder generateDebianPackage() {
		...
		// Rename edited sample (".ex") files before running the build script
		// (manual mode only)
		if (!Variables.isNull("BUNDLE_MODE")
				&& Variables.BUNDLE_MODE.equals(Constants.BUNDLE_MODE_MANUAL)) {
			if (Variables._PACKAGE_CONTENT_FILES_EDITION_STATUS != null) {
				...
				for (File packageSourcesFile : packageSourcesFiles) {
					...
					// In case the file has been edited, remove any possible
					// ".ex" termination
	...
	public static StringBuilder generateRedHatFiles() {
		...
		if (!Files.isPackageSourcesOnDisk()) {
			List<String> commandList = Arguments
					.generateArgumentsForRedHatFiles();
			String command = "bash";
			commandList.add(0, command);
			// Default is "DEB"
	...
	public static void openOutputPath() {
		if (!Variables.BATCH_MODE) {
			...
			Pattern pattern = Pattern
					.compile("The output is located under (.+)/");
